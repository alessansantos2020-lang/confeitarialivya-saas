...
const get = (k) => {
  ...
  return line ? line.slice(k.length + 1).replace(/[\r"]/g, '').trim() : null
}
...
const main = async () => {
  ...
  if (!EMAIL || !SENHA) {
    throw new Error(
      'Faltam SEED_ADMIN_EMAIL e SEED_ADMIN_PASSWORD no .env. ' +
        'Use a conta de dono do sistema.',
    )
  }
  ...
  if (!prod) {
    ;[prod] = await api('products', {
      ...
      body: JSON.stringify({
        name: 'Açaí',
        description:
          'Açaí batido na hora. Escolha o tamanho, as frutas e os complementos.',
        price: 12,
        category_id: cat.id,
        store_id: STORE,
        is_available: true,
