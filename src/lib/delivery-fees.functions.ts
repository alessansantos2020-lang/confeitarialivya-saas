...
export const getDeliveryFees = async (storeId: string = DEFAULT_STORE_ID): Promise<DeliveryFee[]> => {
...
export const getActiveDeliveryFees = async (storeId: string = DEFAULT_STORE_ID): Promise<DeliveryFee[]> => {
  const { data, error } = await supabase
    .from("delivery_fees")
    .select("*")
    .eq("store_id", storeId)
