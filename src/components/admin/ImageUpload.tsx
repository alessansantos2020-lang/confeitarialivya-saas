...
export function ImageUpload({ value, onChange, folder, maxSizeMB, className }: ImageUploadProps) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await uploadFile(file, folder, maxSizeMB);
      onChange(url);
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      {value ? (
        ...
        <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-xl cursor-pointer hover:bg-slate-50 transition-colors ${isUploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
