...
function AuthPage() {
  ...
  const handleLoginSuccess = async (user: { id: string }) => {
    try {
      ...
      const roles = new Set(
        roleRes.data?.map(
          ({ role }: { role: "admin" | "user" | "employee" | "super_admin" }) => role,
        ) ?? [],
      );
      const role = roles.has("super_admin") ? "super_admin" : roles.has("admin") ? "admin" : null;
